...
namespace PvPCommands
{
    public class VSConfig
    {
        ...
        // Default Cooldown Formula: Damage|Heal Amount * 1.2 * (delayed ? 0.80 : 1) * (effect ? 1.3 : 1) * (heal ? 0.6 : 1) seconds. Exceptions: Tickle, Silence, Healing Wish
        ...
        {
            // Example Commands
            // Only Name is required for a command to work, but it won't really do anything. Add Damage and a message to all as a minimum
            new VSCommand("Smash")              // Constructor with Name. Auto-Alias / Auto-Permission will only work this way
            {
                ...
                Description = new List<Message> { new Message("Smashes target player, dealing pvp damage.", Color.Yellow), new Message("Usage: /smash <player>", Color.Yellow) },
                // Message to send when the command is executed. You can choose from MsgAll, MsgPlayer and MsgSelf. Syntax: new Message(" firstmessage ", whether to show user's name (true/false), " secondmessage ", whether to show target's name (true/false), " lastmessage ", Color.ColorName)
            ...
            new VSCommand("Knuckle")
            {
                ...
                Description = new List<Message> { new Message("Punches target player with flare fists, burning them for 15 seconds.", Color.Firebrick), new Message("Usage: /knuckle <player>", Color.Yellow) },
                MsgAll = new Message("", true, " hits ", true, " with his Burning Knuckle!", Color.Firebrick),
                // This is when it gets interesting: You can add effects to commands. First arg is the type (string), second arg is the parameter (duration, delayed damage, etc). Some effects require more than one parameter. Check README.MD for a list of all available effects.
                Effect = new Effect("burn", 15)
            }
        };
